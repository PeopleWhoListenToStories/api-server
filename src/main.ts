...
async function bootstrap() {
  ...
  const options = new DocumentBuilder()
    ...
    .build() // .setBasePath(envConfig.SWAGGER_SETUP_HTTP)
  ...
  await app.listen(port, () => {
    console.log(`listen run in ${swaggerConfig.setupHttp}:${port}\nswagger run in ${swaggerConfig.setupHttp}:${port}${prefix}${swaggerConfig.setupPath}`)
    infoLogger.info(`listen run in ${swaggerConfig.setupHttp}:${port}\nswagger run in ${swaggerConfig.setupHttp}:${port}${prefix}${swaggerConfig.setupPath}`)
  })
  console.log(`[${config.get('server.name')}] 主服务启动成功，端口：${port}`)
}
bootstrap()
